...
<div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
        <h4 class="text-2xl font-bold text-gray-900 dark:text-white">Dashboard Analitik Stok</h4>
    ...
    <div class="flex items-center gap-3 w-full md:w-auto">
        <!-- Filter Bulan/Tahun -->
        <form method="GET" action="{{ route('modul-kapasitas.dashboard') }}" class="flex-grow">
            @php
        ...
        <a href="{{ route('modul-kapasitas.input-harian') }}" class="btn bg-primary text-white hover:bg-blue-700 py-2.5 px-4 rounded-lg flex items-center gap-2 shadow-sm text-sm font-medium whitespace-nowrap">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
            <span class="hidden sm:inline">Lihat Input Harian</span>
        </a>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <!-- Card 1 -->
    <div class="bg-white dark:bg-darkgray rounded-xl p-6 border-t-4 border-t-primary shadow-sm summary-card relative overflow-hidden border border-gray-100 dark:border-gray-700">
        <div class="flex justify-between items-start">
            <div>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Total Stok (Bulan Ini)</p>
                <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($totalStockMonth, 0, ',', '.') }}</h3>
            </div>
    ...
    <!-- Card 2 -->
    <div class="bg-white dark:bg-darkgray rounded-xl p-6 border-t-4 border-t-indigo-500 shadow-sm summary-card relative overflow-hidden border border-gray-100 dark:border-gray-700">
        <div class="flex justify-between items-start">
            <div>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Jumlah Sumber (Source)</p>
                <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $sourcesMonth }} <span class="text-lg font-normal text-gray-500">sumber</span></h3>
            </div>
    ...
    <!-- Card 3 -->
    <div class="bg-white dark:bg-darkgray rounded-xl p-6 border-t-4 border-t-orange-500 shadow-sm summary-card relative overflow-hidden border border-gray-100 dark:border-gray-700">
        <div class="flex justify-between items-start">
            <div>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Kategori Terbanyak</p>
                <h3 class="text-2xl font-bold text-gray-900 dark:text-white truncate max-w-[180px]">{{ $topCategory ? $topCategory->kategori : '-' }}</h3>
            </div>
...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
    ...
    <div class="xl:col-span-2 bg-white dark:bg-darkgray rounded-xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
        <div class="flex justify-between items-center mb-4">
            <h5 class="text-lg font-bold text-gray-900 dark:text-white">Tren Stok Tahunan ({{ $year }})</h5>
            <div class="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-xs font-semibold rounded-full text-primary border border-blue-100 dark:border-blue-800">
                Tahun {{ $year }}
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-darkgray rounded-xl p-0 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col h-full overflow-hidden">
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h5 class="text-lg font-bold text-gray-900 dark:text-white">Distribusi Kategori (Top 7)</h5>
        ...
        <div class="p-6 flex-grow flex flex-col">
            @if(count($distCategories) > 0)
                <div id="distributionChart" class="w-full h-[220px] flex justify-center mb-6"></div>
                
                <div class="space-y-3 flex-grow overflow-y-auto max-h-[180px] custom-scrollbar pr-2">
                    @php
                        ...
                        <div class="category-list-item p-2 rounded-lg transition-colors">
                            <div class="flex justify-between items-center mb-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 truncate max-w-[120px]" title="{{ $kategori }}">{{ $kategori }}</span>
                                </div>
                                <span class="text-sm font-bold text-gray-900 dark:text-white">{{ number_format($total, 0, ',', '.') }}</span>
                            </div>
                            <!-- Progress Bar -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (distCategories.length > 0 && document.getElementById("distributionChart")) {
        ...
        const donutChart = new ApexCharts(document.querySelector("#distributionChart"), donutOptions);
        donutChart.render();
    }
});
</script>

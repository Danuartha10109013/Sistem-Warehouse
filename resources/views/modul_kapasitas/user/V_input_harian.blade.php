...
<div class="card h-full min-w-0 overflow-hidden">
    <div class="card-body min-w-0">
        <div class="mb-4 sm:mb-6">
            ...
            <p class="text-xs sm:text-sm text-gray-500 mt-1">Impor data dari Excel untuk diakumulasikan ke dalam tabel stok harian.</p>
        </div>
                        ...
                        @if(isset($pivot[$groupName]) && count($pivot[$groupName]) > 0)
                            ...
                            @foreach($pivot[$groupName] as $kategori => $days)
                                ...
                                <tr class="border-b border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors group {{ $bgClass }} relative">
                                    ...
                                    @for($d = 1; $d <= $daysInMonth; $d++)
                                        <td class="px-1 sm:px-2 py-2 sm:py-3 text-center border-r border-gray-300 dark:border-gray-600 last:border-r-0 hover:bg-blue-100 dark:hover:bg-gray-500 transition-colors cursor-default text-[11px] sm:text-sm {{ $days[$d] > 0 ? 'text-gray-800 dark:text-gray-200 font-medium' : 'text-transparent' }}">
...
<!-- Modal Input Baru -->
<div id="importModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full p-2 sm:p-4">
    <div class="relative w-full max-w-2xl max-h-full mx-auto">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-darkgray">
            <!-- Modal header -->
            <div class="flex items-start sm:items-center justify-between gap-3 p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white pr-2">
            ...
            <form action="{{ route('modul-kapasitas.input-harian.import') }}" method="POST" enctype="multipart/form-data" class="p-4 md:p-5">
                ...
                <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-600">
                    <button data-modal-hide="importModal" type="button" class="py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 w-full sm:w-auto">Batal</button>
                    <button type="submit" class="text-white bg-primary hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center w-full sm:w-auto">Simpan Data</button>
                </div>
            </form>
        </div>
    </div>

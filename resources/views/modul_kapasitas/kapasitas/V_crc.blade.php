...
<script>
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    title: {
                        display: true,
                        text: ['KAPASITAS CRC (LAYOUT + TRANSIT)', '{{ $months[(int)$month] ?? "" }} {{ $year }}'.toUpperCase()],
                        font: {
                            size: 18,
                            weight: 'bold'
                        },
    ...
    async function exportToExcel() {
        ...
        link.download = "Kapasitas_CRC_Lengkap_{{ $months[(int)$month] ?? '' }}_{{ $year }}.xlsx";
        link.click();
    }
    ...
    function exportToPDF() {
        ...
        const opt = {
            margin:       0.3,
            filename:     'Kapasitas_CRC_{{ $months[(int)$month] ?? '' }}_{{ $year }}.pdf',

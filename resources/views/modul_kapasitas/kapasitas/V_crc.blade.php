    ...
    <div class="card-body">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-white dark:bg-darkgray p-4 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <div class="flex gap-2">
                <button type="button" onclick="exportToExcel()" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2.5 transition-colors shadow-sm flex items-center gap-2 whitespace-nowrap">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    Export Excel
                </button>
                <button type="button" onclick="exportToPDF()" class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2.5 transition-colors shadow-sm flex items-center gap-2 whitespace-nowrap">
        ...
        <div class="relative overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 shadow-md">
            <table class="w-full text-base text-right text-gray-700 dark:text-gray-300" id="crcTable">
                <thead class="text-xs text-white uppercase bg-primary dark:bg-blue-900 shadow-sm">
                    <tr>
                        <th rowspan="2" class="px-6 py-4 text-center border-r border-b border-blue-400 dark:border-blue-700 font-extrabold sticky left-0 bg-primary dark:bg-blue-900 z-10">TGL</th>
                        <th colspan="2" class="px-6 py-2 text-center border-r border-b border-blue-400 dark:border-blue-700 font-extrabold">STOCK</th>
                        <th rowspan="2" class="px-6 py-4 text-center border-r border-b border-blue-400 dark:border-blue-700 font-extrabold">TOTAL STOCK</th>
                        <th rowspan="2" class="px-6 py-4 text-center border-r border-b border-blue-400 dark:border-blue-700 font-extrabold">KAP</th>
                        <th rowspan="2" class="px-6 py-4 text-center border-b border-blue-400 dark:border-blue-700 font-extrabold">TREND</th>
                    </tr>
                    <tr>
                        <th class="px-6 py-2 text-center border-r border-b border-blue-400 dark:border-blue-700 font-extrabold">WH</th>
                        <th class="px-6 py-2 text-center border-r border-b border-blue-400 dark:border-blue-700 font-extrabold">PRD & QA</th>
                    </tr>
                </thead>
                <tbody>
                    @for($d = 1; $d <= $daysInMonth; $d++)
                        ...
                        <tr class="border-b border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors {{ $bgClass }}">
                            <td class="px-6 py-3 text-center font-bold border-r border-gray-300 dark:border-gray-600 sticky left-0 z-10 {{ $bgClass }} group-hover:bg-gray-200 dark:group-hover:bg-gray-600">{{ $d }}</td>
                            <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ $row['wh'] > 0 ? number_format($row['wh'], 3, ',', '.') : '0' }}</td>
                            <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ $row['prd_qa'] > 0 ? number_format($row['prd_qa'], 3, ',', '.') : '0' }}</td>
                            <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600 font-medium">{{ $row['total_stock'] > 0 ? number_format($row['total_stock'], 3, ',', '.') : '0' }}</td>
                            <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ number_format($row['kap'], 3, ',', '.') }}</td>
                            <td class="px-6 py-3 font-medium">{{ number_format($row['trend'], 0, ',', '.') }}%</td>
                        </tr>
                    @endfor
                    
                    <!-- Average Row -->
                    <tr class="bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100 border-b border-gray-400 dark:border-gray-700">
                        <td class="px-6 py-3 text-center font-extrabold border-r border-gray-400 dark:border-gray-700 sticky left-0 bg-gray-200 dark:bg-gray-700 z-10">AVERAGE</td>
                        <td class="px-6 py-3 border-r border-gray-400 dark:border-gray-700 font-bold">{{ number_format($averages['wh'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 border-r border-gray-400 dark:border-gray-700 font-bold">{{ number_format($averages['prd_qa'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 border-r border-gray-400 dark:border-gray-700 font-bold">{{ number_format($averages['total_stock'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 border-r border-gray-400 dark:border-gray-700 font-bold">{{ number_format($averages['kap'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 font-bold">{{ number_format($averages['trend'], 0, ',', '.') }}%</td>
                    </tr>
                    
                    <!-- Highest Row -->
                    <tr class="bg-white dark:bg-darkgray text-red-600 font-bold">
                        <td class="px-6 py-3 text-center border-r border-gray-300 dark:border-gray-600 sticky left-0 bg-white dark:bg-darkgray z-10">Stock tertinggi</td>
                        <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ number_format($highest['wh'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ number_format($highest['prd_qa'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ number_format($highest['total_stock'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3 border-r border-gray-300 dark:border-gray-600">{{ number_format($highest['kap'], 3, ',', '.') }}</td>
                        <td class="px-6 py-3">{{ number_format($highest['trend'], 0, ',', '.') }}%</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
    ...
    function exportToPDF() {
        ...
        const opt = {
            margin:       0.3,
            filename:     'Kapasitas_CRC_Lengkap_{{ $months[$month] ?? '' }}_{{ $year }}.pdf',

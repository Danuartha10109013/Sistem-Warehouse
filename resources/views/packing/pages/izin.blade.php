...
<div class="content-wrapper">
    ...
    <div class="row mb-3">
        ...
        <div class="col-12 col-md-5 d-flex flex-wrap mb-2 mt-2">
            <div class="dropdown  me-2">
                ...
                <ul class="dropdown-menu" aria-labelledby="filterDropdown">
                    <li><a href="{{ route('pemberi-izin', ['filter' => 'today']) }}" class="dropdown-item">Today</a></li>
                    <li><a href="{{ route('pemberi-izin', ['filter' => 'month']) }}" class="dropdown-item">Month</a></li>
                    <li><a href="{{ route('pemberi-izin', ['filter' => 'year']) }}" class="dropdown-item">Year</a></li>
                    <li><a href="{{ route('pemberi-izin', ['filter' => 'all']) }}" class="dropdown-item">All</a></li>
                    <li><a href="{{ route('pemberi-izin', ['filter' => 'Diizinkan']) }}" class="dropdown-item">Sudah Diizinkan</a></li>
                    <li><a href="{{ route('pemberi-izin', ['filter' => 'keluar']) }}" class="dropdown-item">Sudah Keluar</a></li>
                </ul>
            </div>
            ...
            @if ($filter === 'month' && $months->isNotEmpty())
                <select onchange="window.location.href=this.value" class="form-select  me-2">
                    ...
                    @foreach ($months as $month)
                        <option value="{{ route('pemberi-izin', ['filter' => 'month', 'month' => $month->month, 'year' => $month->year]) }}"
            ...
            @elseif ($filter === 'year' && $years->isNotEmpty())
                <select onchange="window.location.href=this.value" class="form-select  me-2">
                    ...
                    @foreach ($years as $year)
                        <option value="{{ route('pemberi-izin', ['filter' => 'year', 'year' => $year->year]) }}"
        ...
        <div class="col-12 col-md-4 mb-2 mt-2" >
            <form action="{{ route('pemberi-izin') }}" method="GET" class="d-flex">
                ...
                @foreach ($data as $d)
                    <tr>
                        ...
                        <td>
                            ...
                            @elseif ($d->status == 1)
                                <label class="text-warning">Diizinkan</label>
                                @else
                        ...
                        <td>
                            ...
                            @if ($d->status == 0)
                                <a href="{{ route('pemberi-izin.setujui', $d->id) }}" class="btn btn-primary ml-2"><i class="fa fa-edit"></i> Beri Izin</a>

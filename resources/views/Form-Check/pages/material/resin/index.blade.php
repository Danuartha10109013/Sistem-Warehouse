...
<div class="col-md-12 container-fluid">
    <div class="content-wrapper">
      ...
      <div class="d-flex justify-content-between align-items-center mb-3">
        <div class="d-flex">
            <a href="{{ Auth::user()->role == 0 ? route('Form-Check.admin.resin.add') : route('Form-Check.pegawai.resin.add') }}"
               class="btn btn-primary mr-2" style="text-decoration: none; font-size: 15px">Tambahkan response</a>
            <a href="{{ route('Form-Check.admin.resin.export') }}"
               class="btn btn-success" style="text-decoration: none; font-size: 15px">Export Excel</a>
        </div>

        <form action="{{ route('Form-Check.admin.resin') }}" method="GET" class="ml-2" style="display: inline;">
            <input type="text" name="search" placeholder="Search By Responden / No Document" class="form-control d-inline" style="width: auto; display: inline;" value="{{ $searchTerm }}">
    ...
    <form action="{{ Auth::user()->role == 0 ? route('Form-Check.admin.resin') : route('Form-Check.pegawai.resin') }}">
      <div class="row mb-3">
          <div class="col-md-5">
                  ...
                  <table class="table">
                    <thead>
                      <tr>
                        ...
                        <th> No Dokumen </th>
                        <th> Responden </th>
                        <th> <a href="{{ route('Form-Check.admin.resin', ['sort' => 'date', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc', 'search' => $searchTerm]) }}">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          @if (Auth::user()->role == 0)
                          <div class="d-flex">
                            <a href="{{route('Form-Check.admin.resin.print', $d->id)}}" class="me-2">
                                <button class="btn btn-success">Print</button>
                            </a>
                            <a href="{{route('Form-Check.admin.resin.show', $d->id)}}" class="me-2">
                            ...
                            document.querySelectorAll('.delete-button').forEach(button => {
                              button.addEventListener('click', function() {
                                  ...
                                  const action = `{{ route('Form-Check.admin.resin.destroy', ':id') }}`.replace(':id', id);
                          ...
                          @else
                          <a href="{{route('Form-Check.pegawai.resin.print', $d->id)}}"> <label class="btn btn-success">print</label></a>
                          @endif
                        </td>
                    </tr>
                    @endforeach
